...
const ConciergeDirectory = () => {
  const { lang, t } = useLanguage();
  const [cityFilter, setCityFilter] = useState("all");

  const { data: companies, isLoading } = useQuery({
    queryKey: ["concierge-all"],
    queryFn: async () => {
  ...
  return (
    <main className="py-16 md:py-24">
      <PageMeta title={t("concierge.title") as string} description={t("concierge.subtitle") as string} />
      <div className="container">
        <div className="mx-auto max-w-2xl text-center mb-12">
          <h1 className="mb-4 text-3xl font-bold text-foreground md:text-4xl">
            {t("concierge.title") as string}
          </h1>
          <p className="text-muted-foreground">{t("concierge.subtitle") as string}</p>
        </div>
        ...
        {isLoading ? (
                  ...
                  <CardFooter>
                    <Button asChild variant="outline" className="w-full gap-2 rounded-full">
                      <Link to={`/concierge/${c.id}`}>
                        {t("concierge.cta.view") as string}
          ...
          <div className="rounded-2xl border border-border bg-card p-12 text-center">
            <p className="text-muted-foreground">{t("concierge.empty") as string}</p>
...
export default ConciergeDirectory;
