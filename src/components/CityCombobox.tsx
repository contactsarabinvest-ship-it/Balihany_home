...
export function CitiesCoveredCombobox({
  ...
  return (
    <div ref={wrapperRef} className={cn("relative space-y-2", className)}>
      ...
      {open && (
        <div className="absolute z-50 mt-1 w-full rounded-md border bg-popover shadow-md p-2 max-h-60 overflow-auto">
          <div className="space-y-1 max-h-44 overflow-auto">
            {filtered.map((city) => (
              <label
                key={city}
                onMouseDown={(e) => e.preventDefault()}
                className="flex items-center gap-2 py-1.5 cursor-pointer hover:bg-muted rounded px-2"
              >
                <Checkbox
